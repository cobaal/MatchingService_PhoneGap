...
$(document).ready(function() {
  ...
  $(".select_gender").click(function() {
    gender_idx = $(".select_gender").index(this);

    $(".select_gender").css("color", "#555555");
    $(".select_gender").css("background", "rgba(0,0,0,0)");

    $(this).css("color", "#FFFFFF");
    $(this).css("background", "#F44949");
  });

  $(".select_grade").click(function() {
    clicked_idx = $(".select_grade").index(this);

    if (clicked_idx == 0) {
      $(".select_grade").css("color", "#555555");
      $(".select_grade").css("background", "rgba(0,0,0,0)");
      grade_idx = [1, 0, 0, 0, 0];

    } else {
      grade_idx[0] = 0;
      grade_idx[clicked_idx] = (grade_idx[clicked_idx] + 1) % 2;

      $("#grade_all_button").css("color", "#555555");
      $("#grade_all_button").css("background", "rgba(0,0,0,0)");
    }

    if (grade_idx[clicked_idx] == 1) {
      $(".select_grade:eq(" + clicked_idx + ")").css("color", "#FFFFFF");
      $(".select_grade:eq(" + clicked_idx + ")" ).css("background", "#F44949");
      
    } else {
      $(".select_grade:eq(" + clicked_idx + ")").css("color", "#555555");
      $(".select_grade:eq(" + clicked_idx + ")" ).css("background", "rgba(0,0,0,0)");
    }

    if (grade_idx == "0,0,0,0,0" || grade_idx == "0,1,1,1,1") 
      $("#grade_all_button").trigger("click");
  });
  ...
  $("#gender_all_button").trigger("click");
  $("#grade_all_button").trigger("click");
